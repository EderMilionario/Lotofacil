    ...
    if st.session_state.data["historico_dados"]:
        ...
        with st.container(border=True):
            ...
            with c1:
                ...
                if tam_atual <= 18:
                    st.success("**Garantia: 100% Exata.** Sem filtros. Ideal para matrizes pequenas.")
                else:
                    st.error("**Status: INVIÁVEL.** Matriz grande demais para cálculo exato.")
            with c2:
                st.markdown("#### 🥈 PLANO B (Híbrido)")
                st.markdown("*Matriz Exata + Poda DNA*")
                if 17 <= tam_atual <= 20:
                    st.info("**Probabilidade: 85-95%.** Base exata + Poda DNA. Elite Probabilística.")
                else:
                    st.warning("**Status: DESLIGADO.** Motor cirúrgico focado em matrizes 17-20.")
            with c3:
                st.markdown("#### 🥉 PLANO B (Heurístico)")
                st.markdown("*Estocástico (Monte Carlo)*")
                if tam_atual > 20:
                    st.info("**Probabilidade: Dinâmica.** Rede de arrasto via Monte Carlo para proteção de banca.")
                else:
                    st.warning("**Status: DESLIGADO.** Motores exatos ativos.")
        ...
        # ==========================================================
        # BÚSSOLA DE APORTE (Otimização de Banca)
        # ==========================================================
        st.markdown("### 🧭 Bússola de Aporte (Recomendação IA)")
        # Lógica de cálculo do ponto ótimo
        jogos_ideal = 1 if tam_atual == 15 else (8 if tam_atual == 17 else (71 if tam_atual == 19 else (210 if tam_atual == 20 else 50)))
        custo_ideal = jogos_ideal * 3.50
        banca_atual = st.session_state.data.get('banca', 0.0)

        if banca_atual >= custo_ideal:
            st.success(f"✅ **Aporte Ideal:** Seu saldo (R$ {banca_atual:.2f}) permite **{jogos_ideal} jogos** (R$ {custo_ideal:.2f}). A IA operará em força máxima.")
        else:
            st.warning(f"⚠️ **Orçamento Defensivo:** O ideal seria **{jogos_ideal} bilhetes** (Custo: R$ {custo_ideal:.2f}). Com R$ {banca_atual:.2f}, a IA ajustará o volume.")
        ...
        # =====================================================================
        # CORREÇÃO DA NOTA: Puxando do IA Real
        ...
        # [DIAGNÓSTICO E DADOS FINAIS MANTIDOS DO SEU CÓDIGO ORIGINAL]
        ...
        if len(historico_painel) >= 2:
            ...
            # TERMÔMETRO E TEIA (Mantenha o seu código original de termômetro e teia aqui)
            # ...
            # PAINEL DE GESTÃO E CONFIANÇA
            ...
            # ANÁLISES EXTRAS (Massa, Linhas, Colunas, Retrospectiva)
            # ... [COLE O RESTO DO SEU CÓDIGO ORIGINAL AQUI] ...
            # ...
            # GRADE DINÂMICA DE PESOS (HEATMAP)
            ...
            for n in range(1, 26):
                ...
                html_pesos += f"<div style='background-color: {bg_color}; padding: 10px; text-align: center; border-radius: 5px; border: 1px solid #ccc;'><b>{n:02d}</b><br><small>Peso: {p_val:.1f}</small></div>"
            html_pesos += "</div>"
